...
The goal of this project is to create a practical solution for optimizing CI/CD workflows. Industry costs for runners can rack up quickly if they are left running constantly. I read that it is more cost-efficient to have AWS host the runners using **EC2 Spot Instances** that die after a job is done. I am currently evaluating whether to use **containers** for this implementation, so the final architecture is still being decided.
